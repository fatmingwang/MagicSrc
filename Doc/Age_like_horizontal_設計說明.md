...
- 常用按鈕與功能：
  ...
  - `工人升級（Upgrade Worker）`：消耗金屬/金錢，提升所有工人的採集效率或移動速度（有冷卻）。
...
- 備註：工廠可有生產效率層級，升級可降低材料消耗或縮短生產時間。
...
說明：上述 JSON 範例為簡化模型，實作時可擴充屬性（抗性、視野、裝備槽、經驗、等級等）。`expToNext` 可視需求改為曲線（例如每級需求以 1.5 倍增加）。若需要，我可以：
- 將這些範例輸出為獨立 JSON/YAML 資料檔；
- 生成對應的 TypeScript 接口或 C# 類別定義；
- 將 `levelUpRules` 標準化為 schema 並產生驗證規則。

### 附錄：每個單位用途說明
下面為文件中列出之示例物件的中文用途與設計要點摘要，可直接用於 UI 的說明字串或設計文件：

- `Farmer`（農夫）
  - 用途：長期穩定產生食物，適合在田地或農場配置。
  - 設計要點：需長時間工作以升級（例：5 分鐘達到一級），升級後可提高採集速率與少量生命值。

- `Fisherman`（漁夫）
  - 用途：在有水域的地圖提高食物採集效率。
  - 設計要點：依賴地形（需水域），升級條件為持續釣魚時間。

- `Miner`（採礦工）
  - 用途：採集金屬與石料，重要於中後期資源供給。
  - 設計要點：可操作採礦機具，升級後採礦效率顯著提升。

- `Knight`（騎士）
  - 用途：前排近戰單位，負責對抗敵方近戰與吸收傷害。
  - 設計要點：透過戰鬥獲取經驗，升級後提升攻擊與耐久。

- `Archer`（弓手）
  - 用途：後排遠程輸出，使用最小與最大射程以保持安全距離。
  - 設計要點：命中敵人可獲經驗，升級可微幅提升射程與攻擊力。

- `Scout`（偵查）
  - 用途：偵查與探索，發現敵情與路徑資訊，避免交戰為主。
  - 設計要點：以時間或發現敵人獲得經驗，升級可增加速度與偵查範圍。

- `School`（學校）
  - 用途：研究科技與解鎖升級，影響整體遊戲策略。
  - 設計要點：建築等級由資源升級，升級可提高研究速度與開放更多隊列。

- `Trainer`（訓練師）
  - 用途：加速訓練與提高單位獲得經驗。
  - 設計要點：透過完成訓練任務獲得經驗，升級可提高加成。

- `Basic Pickaxe`（基礎採礦工具）
  - 用途：裝備給工人以提升採礦效率。
  - 設計要點：通常不具升級系統，通過製造或購買取得。
...
- `Arrow Tower`（箭塔）
  - 用途：自動防禦建築，可遠程打擊入侵敵方單位。
  - 設計要點：升級以資源為主，可透過駐守人員獲得額外效果。

- `Catapult`（投石機）
  - 用途：攻城武器，造成範圍傷害並能打擊遠距目標。
  - 設計要點：需彈藥與多名駐守人員，升級以資源為主。